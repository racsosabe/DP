...
int main(){
	...
	for(int i=0; i<n; i++){
		for(int j=0; j<n; j++){
			cin >> A[i][j];
			if(i > 0) A[i][j] += A[i-1][j]; // Añadimos el cuadro superior
			if(j > 0) A[i][j] += A[i][j-1]; // Añadimos el cuadro del lado izquierdo
			if(i > 0 && j > 0) A[i][j] -= A[i-1][j-1]; // Restamos el cuadro en la diagonal
		}
	}
